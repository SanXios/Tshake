...
f:close()
local tshakee = 'https://api.telegram.org/bot' .. token .. '/sendDocument'
local curl = 'curl "' .. tshakee .. '" -F "chat_id=' .. msg.chat_id_ .. '" -F "document=@' .. 'SanXios.txt' .. '"'
io.popen(curl)
end
...
if is_sudo(msg) then
t = database:get("tshake:name_sudo"..bot_id..msg.chat_id_)  or 'مطور الدرع 👨🏻‍✈️'
...
tshake_oop = database:get("tshake:name_sudo"..bot_id..msg.chat_id_)  or 'مطور الدرع 👨🏻‍💻'
elseif is_creatorbasic(msg) then
...
local t = ""
if is_sudo(msg) then
t = database:get("tshake:name_sudo"..bot_id..msg.chat_id_)  or 'مطور الدرع 👨🏻‍💻'
elseif is_creatorbasic(msg) then
...
if text == 'فكر' and database:get('tshake:'..bot_id..'lock_geam'..msg.chat_id_) then
database:del('tshake:'..bot_id..'l:id1'..msg.chat_id_)
katu = {'قرنية العين','ثلاثة أيام','جزر القمر','الكتاب','الكافر ','الراحة','7','زينب بنت جحش ','40','ما أضيق العيش لولا فسحة الأمل'};
name = katu[math.random(#katu)]
...
name = string.gsub(name,'الكافر','س / ماهو الشئ الذي لايطهر بالماء ويطهر باللسان؟')
...
name = string.gsub(name,'زينب بنت جحش ','س/ من هي السيدة التي أمر الله رسولنا الكريم محمد صلي الله عليه وسلم الزواج منها؟')
